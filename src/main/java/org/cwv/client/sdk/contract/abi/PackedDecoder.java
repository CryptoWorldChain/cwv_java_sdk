...
import org.cwv.client.sdk.contract.abi.util.Integers;

import java.math.BigDecimal;
import java.math.BigInteger;
import java.util.Arrays;

import static org.cwv.client.sdk.contract.abi.ABIType.*;
...
public final class PackedDecoder {
    ...
    private static int decodeBigIntegerArray(ABIType<?> elementType, int arrayLen, byte[] buffer, int idx, Object[] dest, int destIdx) {
        ...
        for (int i = 0; i < arrayLen; i++) {
            BigInteger val = org.cwv.client.sdk.contract.rlp.util.Integers.getBigInt(buffer, idx, len);
            elementType.validate(val);
            bigInts[i] = val;
            idx += len;
        }
    ...
    private static int decodeBigDecimalArray(BigDecimalType elementType, int arrayLen, byte[] buffer, int idx, Object[] dest, int destIdx) {
        ...
        for (int i = 0; i < arrayLen; i++) {
            BigDecimal val = new BigDecimal(org.cwv.client.sdk.contract.rlp.util.Integers.getBigInt(buffer, idx, len), elementType.scale);
            elementType.validate(val);
            bigDecimals[i] = val;
            idx += len;
        }
