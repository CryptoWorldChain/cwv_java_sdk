...
public class ECKey implements Serializable {
    ...
    public static ECKey fromPublicOnly(String pub) throws NoSuchProviderException, NoSuchAlgorithmException,
            InvalidKeySpecException {
        byte[] javaKey_x = EndianHelper.revert(Hex.decode(pub.substring(0, 64)));
        byte[] javaKey_y = EndianHelper.revert(Hex.decode(pub.substring(64, 128)));

        ECParameterSpec ecSpec = ECNamedCurveTable.getParameterSpec("secp256r1");
        ECPoint ecpoint = ecSpec.getCurve().createPoint(new BigInteger(Hex.toHexString(javaKey_x), 16), new BigInteger(Hex.toHexString(javaKey_y), 16));
        ...
        BCECPublicKey pubKey = (BCECPublicKey)kf.generatePublic(pubkeySpec);

        return new ECKey(null, pubKey.getQ());
    }
